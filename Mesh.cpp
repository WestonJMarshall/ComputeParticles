...
Mesh::Mesh(Vertex* vertexArray, unsigned int numVertices, unsigned int* indexArray, unsigned int numIndices, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	indexCount = numIndices;

	CreateBuffers(numVertices, vertexArray, numIndices, indexArray, device);
}

Mesh::Mesh(const char* filePath, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	indexCount = 0;
	...
	CreateBuffers(verts.size(), &verts[0], indexCount, &indices[0], device);
}
...
void Mesh::CreateBuffers(int vertNum, Vertex* verts, int indexNum, unsigned int* indices, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	CalculateTangents(verts, vertNum, indices, indexNum);
	...
	ibd.ByteWidth = sizeof(unsigned int) * indexNum;	// 3 = number of indices in the buffer
